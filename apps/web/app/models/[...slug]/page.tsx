...
import { Layers, Cpu, ShieldCheck, CheckCircle2, ArrowLeft, ArrowRight, Gauge, AlertTriangle, Sparkles } from 'lucide-react';
...
export default async function ModelDetailPage({ params }: PageProps) {
  ...
  const precisions = [
    ...
    { name: 'FP8 (E4M3)', bpp: 1.0, tag: 'fp8', status: 'Near-Lossless (~99.3%)' },
  ...
  const memoryTable = precisions.map((p) => {
    ...
    const kv4kGb = (2 * model.layers * model.kv_heads * model.head_dim * 4096 * (p.bpp <= 1 ? 1.0 : 2.0)) / 1e9;
    const kv32kGb = (2 * model.layers * model.kv_heads * model.head_dim * 32768 * (p.bpp <= 1 ? 1.0 : 2.0)) / 1e9;
  ...
  const representativeGpus = [
    ...
    { slug: 'h100-sxm5-80gb', name: 'NVIDIA H100 SXM5 80GB', precision: 'fp8' as const },
    { slug: 'rtx-4090-24gb', name: 'NVIDIA RTX 4090 24GB', precision: 'int4' as const },
    { slug: 'instinct-mi300x-192gb', name: 'AMD Instinct MI300X 192GB', precision: 'fp8' as const }
  ];

  const fitResults = representativeGpus.map((gpu) => {
    try {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      <div className="flex items-center gap-2 text-xs font-mono text-slate-400">
        <Link href="/models" className="hover:text-white flex items-center gap-1">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 pb-6 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-mono">Arch: {model.architecture}</span>
          </div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight mt-1">{model.name}</h1>
          <p className="text-xs text-slate-400 font-mono mt-1">Repository: {model.id}</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 font-mono text-xs">
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">
          ...
          <span className="text-base font-bold text-white">{model.parameters_billions}B</span>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">
          <span className="text-[10px] text-slate-500 uppercase block">Native Context</span>
          <span className="text-base font-bold text-white">{model.context_window / 1000}k</span>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">
          <span className="text-[10px] text-slate-500 uppercase block">Layers</span>
          <span className="text-base font-bold text-white">{model.layers}</span>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">
          <span className="text-[10px] text-slate-500 uppercase block">KV Heads</span>
          <span className="text-base font-bold text-white">{model.kv_heads}</span>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">
          <span className="text-[10px] text-slate-500 uppercase block">Head Dim</span>
          <span className="text-base font-bold text-white">{model.head_dim}</span>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-3">
          <span className="text-[10px] text-slate-500 uppercase block">License</span>
          <span className="text-base font-bold text-slate-300 truncate block">{model.license}</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-lg font-bold text-white">Memory Requirements by Precision</h2>
          <p className="text-xs text-slate-400">
            Exact physical VRAM requirements including model weights, KV cache at scale, and runtime workspace buffers.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-300">
              {memoryTable.map((row) => (
                <tr key={row.tag} className="hover:bg-slate-800/20">
                  ...
                  <td className="py-3.5 px-4 text-right text-slate-400">{row.kv4kGb} GB</td>
                  <td className="py-3.5 px-4 text-right text-slate-400">{row.kv32kGb} GB</td>
                  <td className="py-3.5 px-4 text-right font-bold text-sky-400">{row.total4kGb} GB</td>
                  <td className="py-3.5 px-4 text-right font-bold text-indigo-300">{row.total32kGb} GB</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-lg font-bold text-white">Representative ModelFit Profiles</h2>
          <p className="text-xs text-slate-400">
            Real-time evaluated ModelFit scores for standard production hardware configurations.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-slate-400 uppercase">{item.precision.toUpperCase()}</span>
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-800 text-[11px]">
                  <div className="flex justify-between">
                    ...
                    <span className={fit.memory_breakdown.is_oom ? 'text-rose-400' : 'text-emerald-400'}>
                      {fit.memory_breakdown.is_oom ? 'OOM Deficit' : `${Math.round((1 - fit.memory_breakdown.vram_utilization_ratio) * 100)}% free`}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Est. Throughput:</span>
                    <span className="text-sky-400 font-semibold">{fit.performance_estimates.estimated_tokens_per_sec} tps</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Est. TPOT:</span>
                    <span className="text-slate-200">{fit.performance_estimates.estimated_tpot_ms} ms</span>
                  </div>
                </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-white">Empirical OpenComputeBench Observations</h2>
            <p className="text-xs text-slate-400">Verified benchmark runs submitted for this model.</p>
          </div>
        </div>
        ...
        {benchmarks.length === 0 ? (
          <div className="rounded-xl border border-slate-800 bg-slate-900/20 p-8 text-center text-xs text-slate-400">
            ...
            <div className="mt-3">
              ...
              >
                Run `modelforge benchmark {model.id}` to submit the first observation →
              </Link>
            </div>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                {benchmarks.map((b) => (
                  <tr key={b.benchmark_id} className="hover:bg-slate-800/20">
                    ...
                    <td className="py-3.5 px-4 uppercase">{b.precision.type}</td>
                    <td className="py-3.5 px-4">{b.runtime.name} {b.runtime.version}</td>
                    <td className="py-3.5 px-4 text-right font-bold text-emerald-400">{b.metrics.tokens_per_second} tok/s</td>
