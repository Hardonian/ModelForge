...
export function compileSLOToDeploymentPlan(
  ...
  for (const device of HARDWARE_CATALOG) {
    ...
    for (const prec of precisions) {
      ...
      const kvGb = (2 * 64 * 8 * 128 * workload.context_length_target * (bpp <= 1.0 ? 1.0 : 2.0) * workload.target_concurrency) / 1e9;
      ...
      if (isNvidia && (isHopperOrBlackwell || device.architecture === 'Ada') && workload.target_concurrency >= 8) {
        ...
        const passesSlo = (!slo.p95_ttft_ms || dynamoTtft <= slo.p95_ttft_ms) &&
                          (!slo.min_tokens_per_second || dynamoTps >= slo.min_tokens_per_second) &&
                          (!slo.max_cost_per_million_tokens_usd || costPerMillion <= slo.max_cost_per_million_tokens_usd);
        ...
        candidates.push({
          ...
          reasons,
          warnings: isDisaggregated ? [] : ['Single-GPU deployment runs in converged mode (no prefill/decode split)']
        });
      }
  ...
  candidates.sort((a, b) => {
    ...
    // Balanced
    return (b.slo_compliance_score + b.confidence_score) - (a.slo_compliance_score + a.confidence_score);
  });
  ...
  return {
    ...
    generated_manifests: {
      ...
      deployment_notes_md: dynamoManifest.deployment_notes_md || nimManifest.deployment_notes_md || vllmManifest.deployment_notes_md
    },
    is_immutable: true
  };
}
