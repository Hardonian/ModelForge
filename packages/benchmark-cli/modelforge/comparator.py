...
def compare_benchmarks(file1: Path, file2: Path, console: Console) -> None:
    ...
    # Model & Config
    table.add_row("Model Repository", b1.model.repository, b2.model.repository, "=" if b1.model.repository == b2.model.repository else "Different")
    table.add_row("Precision", b1.precision.type, b2.precision.type, "=" if b1.precision.type == b2.precision.type else "Different")
    table.add_row("Runtime", f"{b1.runtime.name} {b1.runtime.version}", f"{b2.runtime.name} {b2.runtime.version}", "=" if b1.runtime.name == b2.runtime.name else "Different")
    table.add_row("Hardware", f"{b1.hardware.device} ×{b1.hardware.count}", f"{b2.hardware.device} ×{b2.hardware.count}", "=" if b1.hardware.device == b2.hardware.device else "Different")
    ...
    table.add_row("Tokens / Second", f"{tps1:.1f} tok/s", f"{tps2:.1f} tok/s", f"[{tps_color}]{tps_delta:+.1f}%[/]")
    ...
    table.add_row("P50 TTFT", f"{ttft1:.1f} ms", f"{ttft2:.1f} ms", f"[{ttft_color}]{ttft_delta:+.1f}%[/]")
    ...
    table.add_row("P50 TPOT", f"{tpot1:.1f} ms", f"{tpot2:.1f} ms", f"[{tpot_color}]{tpot_delta:+.1f}%[/]")
    ...
    table.add_row("Peak VRAM", f"{vram1_gb} GB", f"{vram2_gb} GB", f"[{vram_color}]{vram_delta:+.2f} GB[/]")

    table.add_row("Verification Status", b1.verification.status, b2.verification.status, "=" if b1.verification.status == b2.verification.status else "Different")
