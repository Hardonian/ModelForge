...
describe('Hardware Registry & Providers', () => {
  ...
  it('checks runtime support on NVIDIA vs AMD providers', () => {
    const isVllmSupportedOnH100 = nvidiaProvider.isRuntimeSupported('h100-sxm5-80gb', 'vllm');
    assert.strictEqual(isVllmSupportedOnH100, true);

    const isFp8SupportedOnMi300x = amdProvider.isPrecisionSupported('instinct-mi300x-192gb', 'fp8');
