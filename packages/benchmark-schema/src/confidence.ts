 ...
 */
export function calculateEvidenceConfidence(factors: ConfidenceFactors): ConfidenceEvaluation {
  ...
  const sampleVolumeScore = Math.min(10, Math.max(0, factors.benchmarkRunCount * 2));

  // 5. Reproductions (Max 15 pts, 5 pts per reproduction up to 3)
  const reproductionScore = Math.min(15, Math.max(0, factors.reproductionCount * 5));
  ...
  const explanation = `Score ${totalScore}/100 (${grade}) under algorithm v${CONFIDENCE_ALGORITHM_VERSION}: ` +
